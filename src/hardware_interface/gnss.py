...
def get_gnss_data(timeout_attempts=10):
    ...
    try:
        with serial.Serial(port, baudrate, timeout=1) as ser:
            ...
            for attempt in range(timeout_attempts):
                try:
                    ...
                    if parsed_data and 'latitude' in parsed_data and 'longitude' in parsed_data:
                        logger.info(
                            f"GNSS fix: {parsed_data['latitude']:.6f}, {parsed_data['longitude']:.6f}")
            ...
            logger.warning(
                f"No valid GNSS data after {timeout_attempts} attempts")
            return None, None
        ...
        with serial.Serial(port, baudrate, timeout=1) as ser:
            ...
            while True:
                try:
                    ...
                    parsed_data = get_gnss_data()
                    if parsed_data:
                        lat, lon = parsed_data
                        print(f"Latitude: {lat:.6f}, Longitude: {lon:.6f}")
                    else:
                        print("No valid GNSS fix yet.")
