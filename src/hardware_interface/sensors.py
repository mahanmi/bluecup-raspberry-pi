...
# SERIAL_PORT_SENSORS = "/dev/ttyUSB0" # Or your specific port
# BAUD_RATE_SENSORS = 115200


class SensorInterface:
    def __init__(self, communicator: SerialCommunicator):
        """
        Initializes the SensorInterface.
        Args:
            communicator (SerialCommunicator): An instance of SerialCommunicator.
        """
        self.comm = communicator

    def _request_sensor_data(self, sensor_command: str, retries=2, delay=0.1) -> str | None:
        """
        ...
        for attempt in range(retries + 1):
            if self.comm.send_command(sensor_command):
    ...
    def get_temperature(self) -> float | None:
        ...
        command = "S:TEMP?"
        response = self._request_sensor_data(command)
    ...
    def get_imu_data(self) -> dict | None:
        """
...
if __name__ == "__main__":
    ...
    if rov_comm.is_connected():
        sensor_if = SensorInterface(rov_comm)

        print("Requesting Temperature...")
        temp = sensor_if.get_temperature()
        if temp is not None:
        ...
        batt = sensor_if.get_battery_voltage()
        if batt:
